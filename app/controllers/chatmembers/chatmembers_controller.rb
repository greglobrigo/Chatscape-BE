class Chatmembers::ChatmembersController < ApplicationController
  ...
  def add
    ...
    return render json: { status: 'failed', error: 'No members to add' }, status: :unprocessable_entity if participants.nil? || participants.length == 0
  ...
  def leave
    ...
    if chat.nil?
      return render json: { message: 'Chat not found' }, status: :not_found
        ...
        return render json: { status: 'failed', error: 'Cannot leave direct chat' }, status: :unprocessable_entity
    end

    chat_member = ChatMember.find_by(chat_id: chat_id, user_id: current_user.id)
    if chat_member.nil?
      return render json: { status: 'failed', error: 'Chat member not found' }, status: :not_found
    end
  ...
  def validate
    ...
    if chat.nil?
      return render json: { message: 'Chat not found' }, status: :not_found
        ...
        return render json: { status: 'failed', error: 'Cannot add members to direct chat' }, status: :unprocessable_entity
    end
    isAlreadyMember = ChatMember.where(chat_id: chat_id, user_id: participants).exists?
    return render json: { status: 'failed', error: 'A User or users you are trying to add is already a member of the chat.' }, status: :unprocessable_entity if isAlreadyMember
  end
end
